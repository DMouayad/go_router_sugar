...
class CodeGenerator {
  ...
  void _writeHeader(StringBuffer buffer) {
    ...
    buffer.writeln('//');
    buffer.writeln('// 🎯 This file provides a complete routing solution with:');
    ...
    buffer.writeln('//   Navigate.toProduct(id: "123", category: "electronics");');
    buffer.writeln('//   context.goToHome();');
    buffer.writeln('//   context.goToProduct(id: "123");');
    buffer.writeln('//');
    buffer.writeln('// 🚀 Say goodbye to: context.go("/products/123?category=electronics")');
    buffer.writeln('// 🎉 Say hello to:   Navigate.toProduct(id: "123", category: "electronics")');
    buffer.writeln();
  }
  ...
  void _writeImports(StringBuffer buffer) {
    ...
    final imports = routes.map((route) => route.importPath).toSet().toList()..sort();
  ...
  void _writeAppRouter(StringBuffer buffer) {
    ...
    buffer.writeln('///');
    buffer.writeln('/// Generated from your page files with zero configuration required.');
    buffer.writeln('/// Just add this to your MaterialApp.router(routerConfig: AppRouter.router)');
    buffer.writeln('class AppRouter {');
    buffer.writeln('  AppRouter._(); // Private constructor - use static router instance');
    buffer.writeln();
    buffer.writeln('  /// 🎯 The complete GoRouter configuration for your app');
    buffer.writeln('  ///');
    buffer.writeln('  /// Includes ${routes.length} route${routes.length == 1 ? '' : 's'} with full type safety and parameter validation.');
  ...
  void _writeRouteConstants(StringBuffer buffer) {
    ...
    for (final route in routes) {
      ...
      buffer.writeln("  static const String $routeName = '${route.routePath}';");
      buffer.writeln();
    }
  ...
  void _writeNavigationAPI(StringBuffer buffer) {
    ...
    buffer.writeln('///');
    buffer.writeln('/// Navigate to any page with full type safety and parameter validation.');
    ...
    buffer.writeln();
    buffer.writeln('  /// 🧭 The global navigator key for programmatic navigation');
    buffer.writeln('  static final GlobalKey<NavigatorState> _navigatorKey = GlobalKey<NavigatorState>();');
    buffer.writeln();
    buffer.writeln('  /// Access to the navigator key (for advanced use cases)');
    buffer.writeln('  static GlobalKey<NavigatorState> get navigatorKey => _navigatorKey;');
    buffer.writeln();
  ...
  void _writeExtensions(StringBuffer buffer) {
    ...
    buffer.writeln('///');
    buffer.writeln('/// Use these extension methods for more idiomatic Flutter navigation.');
    buffer.writeln('/// Example: context.goToHome() instead of Navigate.toHome()');
  ...
  void _writeGoRoute(StringBuffer buffer, RouteInfo route) {
    ...
    if (routePath.isEmpty || routePath == '//' || routePath.contains('//')) {
      print('⚠️ Warning: Invalid route path format: "${route.routePath}" -> normalized to "/$routePath"');
      ...
      for (final paramName in routeParams) {
        ...
        if (hasParam) {
          ...
          paramMappings.add('$paramName: state.pathParameters[\'$paramName\'] ?? \'\'');
           ...
           print('⚠️ Warning: Route \'$routePath\' has param \':$paramName\' but it\'s not in the ${route.className} constructor.');
  ...
  String _routeToConstantName(String routePath) {
    ...
    final cleanRoute = routePath.startsWith('/') ? routePath.substring(1) : routePath;
  ...
  String _getRouteDescription(RouteInfo route) {
    ...
    if (route.routePath == '/') return '🏠 Home page - Your app\'s starting point';
    
    if (route.parameters.isNotEmpty) {
      final paramDesc = route.parameters.map((p) => 'specific ${p.name}').join(' and ');
  ...
  void _writeNavigationMethod(StringBuffer buffer, RouteInfo route) {
    ...
    if (hasParams) {
      ...
      for (final param in route.parameters) {
        buffer.writeln('  /// [${param.name}] - ${param.isRequired ? 'Required' : 'Optional'} parameter');
      }
    }
    ...
    if (hasParams) {
      ...
      for (final param in route.parameters) {
        ...
        buffer.writeln('    ${required}String${param.isRequired ? '' : '?'} ${param.name},');
      }
      buffer.write('  }');
    }
    ...
    buffer.writeln('      throw StateError(\'Navigator context is null. Make sure AppRouter.router is used in MaterialApp.router\');');
    buffer.writeln('    }');
  ...
  void _writeExtensionMethod(StringBuffer buffer, RouteInfo route) {
    ...
    if (hasParams) {
      final paramList = route.parameters.map((p) => '${p.name}: ${p.name}').join(', ');
