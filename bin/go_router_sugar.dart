...
Future<void> _runGenerate(List<String> args) async {
  ...
  try {
    ...
    print('💡 Try running the command again or check your Flutter project setup.');
    exit(1);
  }
}
...
Future<String> _askPagesPath() async {
  ...
  final existingPaths = commonPaths.where((p) => Directory(p).existsSync()).toList();
...
Future<void> _generateRoutes(String pagesPath, String initialRoute, String outputPath) async {
  ...
  if (pageFiles.isEmpty) {
    ...
    print('💡 Create a page file like: ${path.join(pagesPath, 'home_page.dart')}');
    print('');
...
```
  ...
  final routerContent = _generateRouterContent(pageFiles, pagesPath, initialRoute);
...
Future<void> _runSmartWatch() async {
  ...
  if (configFile.existsSync()) {
    try {
      ...
      print('⚠️  Previous config found but couldn\'t read it. Setting up fresh...');
      print('');
    }
  }
  ...
  await for (final event in watcher) {
    if (event.path.endsWith('_page.dart')) {
      final eventType = event.type == FileSystemEvent.create ? 'Created' :
                       event.type == FileSystemEvent.delete ? 'Deleted' :
                       event.type == FileSystemEvent.modify ? 'Modified' : 'Changed';
...
Future<void> _showVisualRoutes() async {
  ...
  if (!Directory(pagesPath).existsSync()) {
    ...
    print('💡 Run "dart run go_router_sugar generate" first to set up your project.');
    return;
  }
  ...
  print('📄 Would you like to generate a beautiful markdown file with this route map?');
...
void _printRouteTree(String path, Map<String, List<String>> routes, String prefix, bool isLast) {
...
Future<void> _generateRouteMarkdown(List<String> pageFiles, String pagesPath) async {
  ...
  buffer.writeln('');
  buffer.writeln('---');
  buffer.writeln('*Generated on ${DateTime.now().toString().split('.')[0]} by [Go Router Sugar](https://pub.dev/packages/go_router_sugar)*');
...
void _printRouteTreeMarkdown(String path, Map<String, List<String>> routes, String prefix, bool isLast, StringBuffer buffer) {
  if (path.isNotEmpty) {
...
''';
    ...
    await File(path.join(pagesDir.path, 'home_page.dart')).writeAsString(homePageContent);
    ...
    print('');
    print('💡 Pro tip: Use "dart run go_router_sugar watch" for auto-magic route updates!');
...
void _showDetailedHelp() {
  ...
  print('                    Watches your pages folder and updates routes instantly');
  print('');
  print('  🎨 visual (v)     Beautiful route visualization');
  print('                    Shows your route map in the console + generates markdown');
  print('');
  print('  🚀 new           Create instant Flutter app with routing');
...
String _generateRouterContent(List<String> pageFiles, String pagesPath, String initialRoute) {
  final buffer = StringBuffer();
  
  // Header with clear explanation
  buffer.writeln('// 🍬 GENERATED CODE - DO NOT MODIFY BY HAND');
  buffer.writeln('// Generated by Go Router Sugar - The Simplest Flutter Routing Ever!');
  buffer.writeln('// 💡 This file is auto-generated. To add new routes, just create new page files!');
  buffer.writeln('//');
  buffer.writeln('// 📖 How to use:');
  buffer.writeln('// 1. Add to your main.dart: MaterialApp.router(routerConfig: AppRouter.router)');
  buffer.writeln('// 2. Navigate anywhere: Navigate.goToHome(context)');
  buffer.writeln('// 3. Add new pages in your pages folder - routes auto-update!');
  buffer.writeln('');
  ...
  for (final file in pageFiles) {
    ...
    final className = _extractClassName(file);
    buffer.writeln('import \'${relativePath.replaceAll('\\', '/')}\'; // → $className');
  }
  buffer.writeln('');
  
  // Route constants with clear names and documentation
  buffer.writeln('/// 🗺️ Route constants - Use these instead of magic strings!');
  buffer.writeln('/// ');
  buffer.writeln('/// Example: context.go(Routes.home) instead of context.go(\'/home\')');
  ...
  buffer.writeln('/// 🚀 Main router configuration - Your app\'s navigation brain!');
  buffer.writeln('/// ');
  buffer.writeln('/// Use in main.dart: MaterialApp.router(routerConfig: AppRouter.router)');
  buffer.writeln('class AppRouter {');
  buffer.writeln('  /// The configured GoRouter instance - handles all your app navigation');
  ...
  buffer.writeln('/// ');
  buffer.writeln('/// Instead of: context.go(\'/products/123\') ❌ (typo-prone)');
  buffer.writeln('/// Use this:   Navigate.goToProductDetail(context) ✅ (type-safe)');
  ...
  for (final file in pageFiles) {
    ...
    if (hasParams) {
      buffer.writeln('  /// Note: This route needs parameters - implement parameter passing!');
    }
    ...
    if (hasParams) {
      ...
      for (final param in params) {
        buffer.writeln('    required String $param, // 📝 Pass the $param value here');
      }
      buffer.writeln('  }) {');
  ...
  buffer.writeln('  /// and run "dart run go_router_sugar watch" for auto-updates!');
  buffer.writeln('}');
...
String _extractClassName(String filePath) {
  ...
  final parts = baseName.split('_').map((part) {
